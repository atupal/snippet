...
class PingDisk():
	...
	def __read_callback(self, ID, data, data_store):
		...
		if ID in data_store: log.error('PingDisk::read::overlap on %d'%ID)
		data_store[ID] = data
	...
	def read_min(self, index, length):
		length = max(length, self.server.block_size)
		return self.read(index,length)
	...
	def test_region(self, start, end, length, timeout=None):
		if not timeout: timeout = self.safe_timeout()
		log.debug('test_region: region=%d length=%d -> %d'%(start,end,length))
