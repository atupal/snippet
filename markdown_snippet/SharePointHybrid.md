...
这里的输出被重定向到 certverfiy.txt 文件中去了，您可以检查这个文件，查看是否有证据表明这个证书出现了问题。在这个例子中，我们故意检查了一张已经过期了的证书，这是最常见的错误。certverify.txt 文件的内容看起来会像下面这样：

如果证书是证书链的一部分，您会看到像这里列出来的对整个链的检查结果，在这里值得注意的是这一行：ChainContext.dwErrorStatus = CERT_TRUST_IS_NOT_TIME_VALID (0x1), 这表示这个证书的信任不是时间合法的，换句话说就是证书链上的某个东西过期了。
